...
C      DATA VMAXO/-999999./,UIOLD/-99999./,NODOLD/99999/,TOLD/-9999./
...
C CHECK FOR TIME TO END RUN
      ...
      WRITE(77,*)X,",",
     &     Y(26),",",DERY(26),",",TIM(26),",",
     &     UINA(26),",",UIK(26),",",UIP(26),",",UIL(26),",",
     &     Y(JT+4*26+1),",",DERY(JT+4*26+1),",",
     &     Y(JT+4*26+2),",",DERY(JT+4*26+2),",",
     &     Y(JT+4*26+3),",",DERY(JT+4*26+3),",",
     &     Y(JT+4*26+4),",",DERY(JT+4*26+4)
...
c      WRITE(66,501) TIM(NODEZ),(TIM(K),K=NODE1,NODE2)
      WRITE(66,507)'   EPT ' ,(EPT(K),K=NODE1,NODE2)
      ...
      ENDIF
  507 FORMAT('  ',A7,3X,10(2X,ES10.3))  ! Change from 8 to 10 (12/17/95)
...
c      ENDDO
 16   IF (IPT-1) 25,25,50
   25 NODE2 = NODE1 + 9   ! Changed from 8 to 9, 12/23/08, for data.out
      ...
      WRITE(66,507)'   EPT ' ,(EPT(K),K=NODE1,NODE2)
      WRITE(66,507)'   TIM ' ,(TIM(K),K=NODE1,NODE2)
      WRITE(66,507)'   UINA', (UINA(K),K=NODE1,NODE2)
      WRITE(66,507)'   UIK ', (UIK(K),K=NODE1,NODE2)
      WRITE(66,507)'   UIP ', (UIP(K),K=NODE1,NODE2)
      WRITE(66,507)'   UIL ', (UIL(K),K=NODE1,NODE2)
      WRITE(66,507)'   SUMK', (SUMK(K),K=NODE1,NODE2)
      WRITE(66,507)'   H   ', (VALH(K),K=NODE1,NODE2)
      WRITE(66,507)'   M   ', (VALM(K),K=NODE1,NODE2)
      WRITE(66,507)'   P   ', (VALP(K),K=NODE1,NODE2)
      WRITE(66,507)'   N   ', (VALN(K),K=NODE1,NODE2)
      WRITE(66,507)'   dH  ', (DERH(K),K=NODE1,NODE2)
      WRITE(66,507)'   dM  ', (DERM(K),K=NODE1,NODE2)
      WRITE(66,507)'   dP  ', (DERP(K),K=NODE1,NODE2)
      WRITE(66,507)'   dN  ', (DERN(K),K=NODE1,NODE2)
      IF(VMAX .EQ. VMAXO .AND. UIO .EQ. UIOLD
     &    .AND. NODOLD .EQ. NODE .AND. TOLD .EQ. TMAX)THEN
        GOTO 50
      ELSE
